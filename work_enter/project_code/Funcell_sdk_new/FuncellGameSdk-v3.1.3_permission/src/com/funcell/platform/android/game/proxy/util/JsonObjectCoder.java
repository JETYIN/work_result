...
public class JsonObjectCoder {
	...
	public static ParamsContainer jsonToParamsContainer(String input, Void condition){
		...
		try {
			...
			for (Iterator<String> keysIterator = jsonObject.keys(); keysIterator.hasNext();) {
				...
				Object value = jsonObject.get(key);
				if (value instanceof JSONObject) {
					paramsContainer.put(key, decode(value.toString(), condition));
				} else if (value instanceof JSONArray) {
					paramsContainer.put(key,JsonListCoder.decode(value.toString(), condition));
				} else {
					paramsContainer.put(key, value);
				}
			}
		} catch (JSONException e) {
			e.printStackTrace();
	...
	public static String paramsContainerToJson(ParamsContainer input){
		return new JSONObject(input).toString();
	}
}
