...
public class FuncellPermissionsManager {
	...
	/**
	 * 构造方法,返回一个可用的FuncellPermissionsManager
	 *
	 * @param object
	 *            请求界面的Context,有可能是Activity,Fragment,android.app.Fragment
	 * @param permissionsCallbacks
	 *            监视用户提供权限的事件
	 * @param rationale4ReqPer
	 *            询问是否前往请求权限的提示语
	 * @param rationale4NeverAskAgain
	 *            询问在之前选择了"不再询问"之后是否选择去设置里修改权限的提示语
	 * @param positiveBtn4ReqPer
	 *            询问是否前往请求权限时Positive Button上的String的Res id
	 * @param negativeBtn4ReqPer
	 *            询问是否前往请求权限时Negative Button上的String的Res id
	 * @param positiveBtn4NeverAskAgain
	 *            询问在之前选择了"不再询问"之后是否选择去设置里修改权限时 Positive Button上的String的Res id
	 * @param negativeBtn4NeverAskAgain
	 *            询问在之前选择了"不再询问"之后是否选择去设置里修改权限时 Negative Button上的String的Res id
	 * @param requestCode
	 *            请求码
	 */
	...
	private void requestPermissions(final Object object, String rationale, @StringRes int positiveButton,
		...
		if (!hasPermissions(getActivity(object), perms)) {// 大于23
	...
	private boolean shouldShowRequestPermissionRationale(Object object, String perm) {
		if (object instanceof Activity) {
			...
			// //23中才会有shouldShowRequestPermissionRationale
			return false;
		} else {
			return false;
		}
