...
#                 return
        ...
        for key in self.__jsonChannelList.keys():
            if item.text() == self.__jsonChannelList[key]:
                ...
                if not os.path.exists(channelSdkDir) or not os.path.exists('Log/'+productcode+'/ChannelVersionFile.log'): #如果没有这个配置文件，就使用json数据里面的
...
#                     print '_pluginVersion',_pluginVersion
...
#         
    ...
    def saveChannelSetting_multidex(self,chanelData,dialog):
        ...
        if dialog.checkBox.isChecked():
            checkBoxflag = "True"
        info = channel+'|'+dexNum+'|'+checkBoxflag
        logDir = 'Log/'
        productcode = ConfigParse.shareInstance().getProductCode()
        if not os.path.exists(logDir+productcode):
            os.makedirs(logDir+productcode)
        ...
        if os.path.exists(logDir +productcode+ '/ChannelSettingMultidexFile.log'):
            ...
            for line in logFile.readlines():
                ...
                if Info1[0] == Info2[0] and Info1[1] == Info2[1]:
                    flag = True
                    oldstr = Info1[2] +'|'+ Info1[3]
                    newstr = Info2[2] +'|'+ Info2[3]
            logFile.close()
            if flag:
...
#         print 'versionList:',versionList
        ...
        for key in versionList:
            ...
            description = u'依赖最低FuncellSDK版本:%s'%(dependency)
            try:
...
#             item.label_3.setText(u'依赖最低FuncellSDK版本:%s'%(dependency))
            item.label_3.setText(description)
