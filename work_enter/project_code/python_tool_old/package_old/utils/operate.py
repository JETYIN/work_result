...
#     if os.path.exists(drawable_hdpi_v4):
#         hdpiIcon.save(os.path.join(drawable_hdpi_v4, gameIconName), 'PNG')
#     if os.path.exists(drawable_ldpi_v4):
#         ldpiIcon.save(os.path.join(drawable_ldpi_v4, gameIconName), 'PNG' )
#     if os.path.exists(drawable_mdpi_v4):
#         mdpiIcon.save(os.path.join(drawable_mdpi_v4, gameIconName), 'PNG')
#     if os.path.exists(drawable_xhdpi_v4):
#         xhdpiIcon.save(os.path.join(drawable_xhdpi_v4, gameIconName), 'PNG')
#     if os.path.exists(drawable_xxhdpi_v4):
#         xxhdpiIcon.save(os.path.join(drawable_xxhdpi_v4, gameIconName), 'PNG')
#     if os.path.exists(drawable_xxxhdpi_v4):
#         xxxhdpiIcon.save(os.path.join(drawable_xxxhdpi_v4, gameIconName), 'PNG')
    
#     if os.path.exists(drawable_hdpi_v4):
#         hdpiIcon.save(os.path.join(drawable_hdpi_v4, gameIconName), 'PNG')
#     else:
#         os.mkdir(drawable_hdpi_v4)
#         hdpiIcon.save(os.path.join(drawable_hdpi_v4, gameIconName), 'PNG')
#         
#     if os.path.exists(drawable_ldpi_v4):
#         ldpiIcon.save(os.path.join(drawable_ldpi_v4, gameIconName), 'PNG' )
#     else:
#         os.mkdir(drawable_ldpi_v4)
#         ldpiIcon.save(os.path.join(drawable_ldpi_v4, gameIconName), 'PNG' )
#         
#     if os.path.exists(drawable_mdpi_v4):
#         mdpiIcon.save(os.path.join(drawable_mdpi_v4, gameIconName), 'PNG')
#     else:
#         os.mkdir(drawable_mdpi_v4)
#         mdpiIcon.save(os.path.join(drawable_mdpi_v4, gameIconName), 'PNG')
#         
#     if os.path.exists(drawable_xhdpi_v4):
#         xhdpiIcon.save(os.path.join(drawable_xhdpi_v4, gameIconName), 'PNG')
#     else:
#         os.mkdir(drawable_xhdpi_v4)
#         xhdpiIcon.save(os.path.join(drawable_xhdpi_v4, gameIconName), 'PNG')
#         
#     if os.path.exists(drawable_xxhdpi_v4):
#         xxhdpiIcon.save(os.path.join(drawable_xxhdpi_v4, gameIconName), 'PNG')
#     else:
#         os.mkdir(drawable_xxhdpi_v4)
#         xxhdpiIcon.save(os.path.join(drawable_xxhdpi_v4, gameIconName), 'PNG')
#         
#     if os.path.exists(drawable_xxxhdpi_v4):
#         xxxhdpiIcon.save(os.path.join(drawable_xxxhdpi_v4, gameIconName), 'PNG')
#     else:
#         os.mkdir(drawable_xxxhdpi_v4)
#         xxxhdpiIcon.save(os.path.join(drawable_xxxhdpi_v4, gameIconName), 'PNG')
    
    
    if not os.path.exists(drawablePath):
        os.mkdir(drawablePath)
    if not os.path.exists(drawable_hdpi_v4):
        os.mkdir(drawable_hdpi_v4)
    if not os.path.exists(drawable_ldpi_v4):
        os.mkdir(drawable_ldpi_v4)
    if not os.path.exists(drawable_mdpi_v4):
        os.mkdir(drawable_mdpi_v4)
    if not os.path.exists(drawable_xhdpi_v4):
        os.mkdir(drawable_xhdpi_v4)
    if not os.path.exists(drawable_xxhdpi_v4):
        os.mkdir(drawable_xxhdpi_v4)
    if not os.path.exists(drawable_xxxhdpi_v4):
        os.mkdir(drawable_xxxhdpi_v4)
...
def decompileApk_android(apkFile, targetDir, lock, apkTool = 'apktool2.jar'):
    ...
    if not os.path.exists(targetDir):
        os.makedirs(targetDir)
    cmd = '"%s" -jar -Xms512m -Xmx512m "%s" -q d -b -f "%s" -o "%s" -p "%s"' % (file_operate.getJava(),
     apkTool,
     apkFile,
     targetDir,
     frameworkDir)
...
def recompileApk(srcFolder, apkFile, apkTool = 'apktool2.jar'):
    ...
    if os.path.exists(srcFolder):
        cmd = '"%s" -jar -Xms512m -Xmx512m "%s" -q b -f "%s" -o "%s" -p "%s"' % (file_operate.getJava(),
         apkTool,
         srcFolder,
         apkFile,
         frameworkDir)
...
def splitDex_old(workDir, decompileDir):
    """
    ...
    for f in allFiles:
        f = f.replace("\\", "/")
        if "com/raink/korea/platform/android" in f or "/com/haowan" in f or "/com/haowan123" in f or "/com/funcell123" in f or "/android/support/multidex" in f or "com/android/vending" in f or "com/funcell/platform/android/game/proxy" in f or "com/yunva/video/sdk/YunvaVideoTroops" in f:
...
def splitDex(workDir, decompileDir):
    ...
    for f in allFiles:
        f = f.replace("\\", "/")
        if "/com/haowan" in f or "/com/haowan123" in f or "/com/funcell123" in f or "/android/support/multidex" in f or "com/android/vending" in f or "com/funcell/platform/android/game/proxy" in f or "com/nextmv" in f or "com/yunva" in f:
#             currFucNum = currFucNum + file_operate.get_smali_func_num(f)
