...
public class FunLogStatsUtils {
	...
	public static class LogTaskRunnable implements Runnable {
		...
		public String getTotalTime() {
			long totalTime = Long.valueOf(HWUtils.getTimestamp()) - Long.valueOf(HWUtils.beginTime);
			return totalTime + "";
		}
		...
		public void run() {
			...
			postData.put("operators", HWUtils.getMobileServiceProvider(mContext));
			...
			String logResponse = HWHttpResponse.doSendHttpPostResponse(LOG_URL, postData);
			...
			HWUtils.logError("HWSDKLOG", "errorCode = " + errorCode + " , message = " + message);
		...
		public String getSign(LogTaskInfo logTaskInfo) {
			...
			if (null != accessToken && 0 < accessToken.trim().length()) {
				signBuilder.append("access_token=").append(accessToken).append("&");
			}
			...
			signBuilder.append("cp_id=").append(FunCellPlatformSdkApi.getInstance().mAppId).append("&");
			signBuilder.append("cp_version=").append(HWUtils.getAppVersionName(mContext)).append("&");
			signBuilder.append("create_time=").append(HWUtils.getTimestamp()).append("&");

			signBuilder.append("device_id=").append(HWUtils.getDeviceUID(mContext)).append("&");

			signBuilder.append("event_code=").append(mLogTaskInfo.getEventCode()).append("&");

			if (null != extData && 0 < extData.trim().length()) {
				signBuilder.append("ext_data=").append(extData).append("&");
			}

			signBuilder.append("language=").append(HWUtils.getLocalLangage()).append("&");
			signBuilder.append("model=").append(HWUtils.getPhoneModel()).append("&");
			signBuilder.append("network=").append(HWUtils.getNetType(mContext)).append("&");

			signBuilder.append("operators=").append(HWUtils.getMobileServiceProvider(mContext)).append("&");
			signBuilder.append("os=").append(HWUtils.getPhoneVersion()).append("&");

			signBuilder.append("platform=").append(HWUtils.getChannelCode(mContext)).append("&");
			signBuilder.append("resolution=").append(HWUtils.getScreenPiexl(mContext)).append("&");
			signBuilder.append("total_time=").append(getTotalTime());
			signBuilder.append("#").append(FunCellPlatformSdkApi.getInstance().mAppKey);
			;
	...
	public static class AtsLogTaskRunnable implements Runnable {
		...
		public void run() {
			...
			String logResponse = HWHttpResponse.doSendHttpPostResponse(ATS_LOG_URL + method, postData);
